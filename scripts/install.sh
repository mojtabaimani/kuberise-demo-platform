...
function install_argocd() {
  ...
  helm upgrade \
    ...
    --create-namespace  \
    --wait \
    -f values/defaults/argocd/values.yaml \
    -f "$values_file" \
...
function configure_oidc_auth() {
  ...
  local domain=$3
  local cluster_name=${4:-"default"}  # Added cluster_name parameter with default value
...
cluster_name=${2:-onprem}                               # example: onprem, dta, azure etc. (default: local)
...
VALUES_FILE="values/$cluster_name/argocd/values.yaml"
...
configure_oidc_auth "$CONTEXT" "$kubernetes_secret" "$DOMAIN" "$cluster_name"  # Pass cluster_name as the fourth parameter
